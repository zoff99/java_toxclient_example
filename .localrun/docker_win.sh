...
for system_to_build_for in $build_for ; do
    ...
    system_to_build_for=$(echo "$system_to_build_for_orig" 2>/dev/null|tr ':' '_' 2>/dev/null)"_win"
...
if [ "$1""x" == "localx" ]; then
    ../circle_scripts/deps_win.sh local || exit 1
    ../circle_scripts/java_jni_lib_win.sh local || exit 1
else
