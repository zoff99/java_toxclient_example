#! /bin/bash
#
# [TRIfA], shellscript part of Tox Reference Implementation for Desktop
...
while [ $i -lt $nodes_count ]; do
        ...
        if [ "$status_udp""x" == "true""x" ]; then
              if [ "$ipv4""x" != "NONEx" ]; then
                echo -n 'MainActivity.bootstrap_single_wrapper("'"$ipv4"'",'"$port"',"'"$pubkey"'");' >> "$outfile"
                echo "node #""$i:"
...
while [ $i -lt $nodes_count ]; do
        ...
        if [ "$status_tcp""x" == "true""x" ]; then
            if [ "$port""x" != "x" ]; then
              if [ "$ipv4""x" != "NONEx" ]; then
                echo -n 'MainActivity.add_tcp_relay_single_wrapper("'"$ipv4"'",'"$port"',"'"$pubkey"'");' >> "$outfile"
                echo "node #""$i:"
                if [ $[ $i + 1 ] != $good_nodes_count ]; then
                        echo -n '' >> "$outfile"
                fi
