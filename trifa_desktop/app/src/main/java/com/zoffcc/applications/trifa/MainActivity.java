...
public class MainActivity extends JFrame implements WindowListener, WindowFocusListener, WindowStateListener
{
    ...
    static final String Version = "1.0.45";
    // --------- global config ---------
    // --------- global config ---------
        ...
        try
        {
            ...
            else if (ft.kind == TOX_FILE_KIND_FTV2.value)
            {
                ...
                else
                {
                    ...
                    ByteBuffer file_chunk = ByteBuffer.allocateDirect((int) (file_chunk_length + TOX_FILE_ID_LENGTH));
        ...
        else // normal chunck recevied ---------- (NOT start, and NOT end)
        {
            try
            {
                try
                {
                    ...
                    {
                        fos.seek(position);
                        fos.write(Arrays.copyOfRange(data, TOX_FILE_ID_LENGTH, data.length));
                        fos.close();
                    }
                    else
                    {
